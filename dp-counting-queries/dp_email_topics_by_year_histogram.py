"""
Script to calculate the number of emails sent by topic and by year, writes to disk.
"""
import pandas as pd
import numpy as np
import json
...
# delta of one month
d_w = datetime.timedelta(days=365.25)
u2 = u1 + d_w
...
f = open("./dp_count_email_by_topic_and_year.txt", "w")
f.write(json.dumps(dp_counts))
f.close()
f = open("./normalized_dp_count_email_by_topic_and_year.txt", "w")
